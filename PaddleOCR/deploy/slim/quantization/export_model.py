...
def export_single_model(quanter, model, infer_shape, save_path, logger):
    quanter.save_quantized_model(
        model, save_path, input_spec=[paddle.static.InputSpec(shape=[None] + infer_shape, dtype="float32")]
    )
...
def main():
    ...
    infer_shape = [3, 32, 100] if config["Architecture"]["model_type"] != "det" else [3, 640, 640]
    ...
    if arch_config["algorithm"] in [
        ...
        for idx, name in enumerate(model.model_name_list):
            ...
            export_single_model(quanter, model.model_list[idx], infer_shape, sub_model_save_path, logger)
    else:
