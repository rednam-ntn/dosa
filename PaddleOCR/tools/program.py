        ...
        for idx, batch in enumerate(train_dataloader):
            ...
            if (
                ...
                if model_average:
                    ...
                    Model_Average.apply()
                cur_metric = eval(model, valid_dataloader, post_process_class, eval_class, model_type, use_srn=use_srn)
                ...
                if vdl_writer is not None:
                    vdl_writer.add_scalar(
                        "EVAL/best_{}".format(main_indicator), best_model_dict[main_indicator], global_step
                    )
            global_step += 1
            optimizer.clear_grad()
            batch_start = time.time()
...
def eval(model, valid_dataloader, post_process_class, eval_class, model_type, use_srn=False):
    model.eval()
    with paddle.no_grad():
        total_frame = 0.0
        total_time = 0.0
