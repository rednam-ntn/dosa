...
from tools.infer.utility import draw_boxes, draw_ocr
    ...
    for image_file in image_file_list:
        ...
        if is_visualize:
            ...
            if draw_img is not None:
                ...
                cv2.imwrite(os.path.join(draw_img_save, os.path.basename(image_file)), draw_img[:, :, ::-1])
                logger.info(
                    "The visualized image saved in {}".format(os.path.join(draw_img_save, os.path.basename(image_file)))
                )
        cnt += 1
        if cnt % 100 == 0:
