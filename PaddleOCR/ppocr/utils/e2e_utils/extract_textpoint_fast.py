...
def ctc_decoder_for_image(gather_info_list, logits_map, Lexicon_Table, pts_num=6):
    ...
    for gather_info in gather_info_list:
        if len(gather_info) < pts_num:
            continue
        dst_str, xys_list = instance_ctc_greedy_decoder(gather_info, logits_map, pts_num=pts_num)
...
def generate_pivot_list_fast(p_score, p_char_maps, f_direction, Lexicon_Table, score_thresh=0.5):
    """
    ...
    decoded_str, keep_yxs_list = ctc_decoder_for_image(all_pos_yxs, logits_map=p_char_maps, Lexicon_Table=Lexicon_Table)
