...
def create_config_log_dict(config, dataset_train, dataset_val, args):
    ...
    config_log = {
        ...
        "args": vars(args),
        "dataset_train": train_images,
        "dataset_val": val_images,
    }
    return config_log
...
def get_args():
    ...
    parser = argparse.ArgumentParser(description="Train Mask R-CNN on a Document Annotation dataset.")
    parser.add_argument("command", help="'train' or 'evaluate' on dataset")
    parser.add_argument(
        "--train-dataset",
        required=True,
        help="Directory of the Document Annotation dataset",
    )
    parser.add_argument("--train-version", required=True, help="Annotation file version")
    parser.add_argument("--train-version2", help="Optional second train annotation file version")
    parser.add_argument(
        "--val-dataset",
        required=True,
        help="Directory of the Document Annotation dataset",
    )
    parser.add_argument("--val-version", required=True, help="Annotation file version (default=man)")
    parser.add_argument("--epochs-offset", type=int, default=0, help="Number of total epochs")
    parser.add_argument("--epochs1", type=int, default=20, help="Number of total epochs")
    parser.add_argument("--epochs2", type=int, default=60, help="Number of total epochs in stage 2")
    parser.add_argument("--epochs3", type=int, default=80, help="Number of total epochs in stage 3")
    parser.add_argument("--gpu-count", type=int, default=1, help="Number gpus to use")
    parser.add_argument(
        "--augmentation",
        action="store_true",
        help="Image augmentation (right/left flip 50% of the time)",
    )
    parser.add_argument("--chargrid", action="store_true", help="use chargrid images")
    parser.add_argument(
        "--only-multicells",
        action="store_true",
    ...
    parser.add_argument("--resize-mode", type=str, default="square", help="resizing method for images")

    parser.add_argument(
        "--classes",
        type=str,
        nargs="+",
        help="Specify the classes to train on",
        required=True,
    )
    parser.add_argument("--model", required=True, help="Path to weights .h5 file or 'coco'")
    parser.add_argument("--logs", required=True, help="Logs and checkpoints directory (default=logs/)")
    ...
    parser.add_argument("--manualseed", type=int, help="manual random seed for subset selection")
    parser.add_argument(
        "--numimgs",
        type=int,
