...
def get_all_bbox_anns_for_current_id(parent_ann_id, ann_children_ids, ann_by_id, page=None):
    ...
    if page is None:
        ...
        all_bbox_anns = [
            ann_by_id[ann_id]
            for ann_id in all_children_ids
            if ann_by_id[ann_id]["category"] == "box" and ann_by_id[ann_id]["page"] == page
        ]
...
def create_annotations_to_add(annotations_path, only_multicells, only_labelcells, classes, page):
    ...
    if only_multicells is True or only_labelcells is True:
        ...
        for ann in annotations_list:
            if ann["category"] == "table_cell":
                ...
                if (
                    only_multicells is True
                    and col_range is not None
                    and col_range[0] is not None
                    and (col_range[1] > col_range[0])
                ):
                    multicell_count += 1
                    ...
                    and row_range[0] is not None
                    and (row_range[1] > row_range[0])
                ):
                    multicell_count += 1
                    new_annotation_list.append(ann)
                elif only_labelcells is True and (
                    (col_range is not None and col_range[0] is not None and col_range[0] == 0)
                    or (row_range is not None and row_range[0] is not None and row_range[0] == 0)
                ):
                    new_annotation_list.append(ann)
            else:
                new_annotation_list.append(ann)
    ...
    for annotation_class in classes:
        for annotation_of_allowed_class in anns_by_cat[annotation_class]:
            ...
            all_bboxes_for_current_annotation = get_all_bbox_anns_for_current_id(
                annotation_of_allowed_class["id"], ann_children_ids, ann_by_id, page
            )
...
def get_random_entry_subset(entry_tuples, manualseed, subset_numimgs):
    ...
    logger.debug(
        "subselected {} random images from total of {}:".format(
            subset_numimgs, len(entry_random_subset), entry_random_subset
        )
    )
...
class DocsDataset(mrcnn_utils.Dataset):
    ...
    def load_docs(
        ...
        dataset_dir,
        version,
        classes=["table", "table_caption", "tabular", "table_cell"],
        subset_sample=False,
        subset_numimgs=100000,
        manualseed=None,
        only_multicells=False,
        only_labelcells=False,
    ):
        ...
        entry_tuples = find_available_documents(
            dataset_dir=dataset_dir,
            version=version,
            subset_sample=subset_sample,
            subset_numimgs=subset_numimgs,
    ...
    def load_mask(self, image_id):
        ...
        generated_annotations = create_annotations_to_add(
            annotations_path, only_multicells, only_labelcells, classes, page
        )
        ...
        return (
            ...
            bboxes,
            np.array(ann_ids).astype(np.int32),
        )
