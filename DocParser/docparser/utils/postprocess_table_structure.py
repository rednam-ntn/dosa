...
def adjust_and_move_row_and_column_borders(annotations):
    ...
    if len(col_nrs) > 1:
        for col_nr in col_nrs[:-1]:
            ...
            col_anns[col_nr]["bbox"] = [
                cur_col_x0,
                cur_col_y0,
                cur_col_x1 - cur_col_x0,
                cur_col_y1 - cur_col_y0,
            ]
            col_anns[next_col_nr]["bbox"] = [
                next_col_x0,
                next_col_y0,
                next_col_x1 - next_col_x0,
    ...
    if len(row_nrs) > 1:
        for row_nr in row_nrs[:-1]:
            ...
            row_anns[row_nr]["bbox"] = [
                cur_row_x0,
                cur_row_y0,
                cur_row_x1 - cur_row_x0,
                cur_row_y1 - cur_row_y0,
            ]
            row_anns[next_row_nr]["bbox"] = [
                next_row_x0,
                next_row_y0,
                next_row_x1 - next_row_x0,
...
def create_all_cells_and_assign_coordinates(annotations):
    ...
    for cell_ann in detected_cells:
        ...
        if len(row_overlaps) > 0 and len(col_overlaps) > 0 and (len(row_overlaps) > 1 or len(col_overlaps) > 1):
            ...
            cell_ann["properties"] = "{}-{},{}-{}".format(
                row_overlaps[0], row_overlaps[-1], col_overlaps[0], col_overlaps[-1]
            )
    ...
    if len(normal_cells_delete_ids) > 0:
        logger.debug(
            "deleting {} detected normal cells (using row/col intersections instead".format(
...
def second_bbox_contained_in_first_bbox_xyformat(b1, b2, tolerance=0):
    return (
        b1[0] - tolerance <= b2[0]
        and b1[1] - tolerance <= b2[1]
        and b1[2] + tolerance >= b2[2]
        and b1[3] + tolerance >= b2[3]
    )
...
def second_bbox_contained_in_first_bbox(b1, b2, tolerance=0):
    return (
        b1[0] - tolerance <= b2[0]
        and b1[1] - tolerance <= b2[1]
        and b1[0] + b1[2] + tolerance >= b2[0] + b2[2]
        and b1[1] + b1[3] + tolerance >= b2[1] + b2[3]
    )
