...
from docparser.utils.postprocess_table_structure import (
    adjust_and_move_row_and_column_borders,
    adjust_bboxes_to_true_page_coordinates,
    assign_numbers_to_rows_and_cols,
    create_all_cells_and_assign_coordinates,
    limit_all_annotations_to_crop_region,
    resolve_direct_nesting_of_rows_and_columns,
)
from docparser.utils.postprocess_utils import (
    generate_xmls_per_doc,
    get_crop_region,
    get_detections_for_directory,
    retrieve_text_elements_for_page,
)

logger = logging.getLogger(__name__)


def process_detections_for_icdar_data(
    detections_dir,
    output_xml_dir,
    ...
    for doc_name in detections_per_doc.keys():
        ...
        original_doc_images = {
            int(x.rsplit("-")[-1].split(".png")[0]): os.path.join(doc_subdir, x)
            for x in os.listdir(doc_subdir)
            if x.endswith(".png") and not "char" in x
        }
        ...
        for page_nr in detections_per_doc[doc_name].keys():
            for table_nr in detections_per_doc[doc_name][page_nr].keys():
                sorted_region_nrs = sorted(
                    list(detections_per_doc[doc_name][page_nr][table_nr]),
                    key=lambda x: int(x),
                )
                for region_nr in sorted_region_nrs:
                    [crop_x0, crop_y0, crop_x1, crop_y1] = get_crop_region(
                        crop_data_folder,
                        doc_name,
                        page_nr,
                        table_nr,
