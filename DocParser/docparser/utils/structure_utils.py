...
def get_sorted_sequence_of_nodes(doc_graph, sibling_ids):
    ...
    root_nodes = [
        node for node, in_degree in sibling_subgraph.in_degree() if in_degree == 0
    ]  # root node: node which has no incoming edges
    if len(root_nodes) != 1:
        raise AssertionError(
            "nodes with in_degree 0: {}, all sibling nodes passed: {}, subgraph nodes: {}, subgraph edges: {}".format(
                root_nodes,
                sibling_ids,
                list(sibling_subgraph.nodes()),
                list(sibling_subgraph.edges()),
            )
        )
...
def create_flat_annotation_list(bboxes, meta_bboxes, hierarchy_relations, sequence_relations):
    ...
    logger.debug(
        "{}/{} size for all bbox/relation ids: {}, {}".format(
            len(all_non_meta_bbox_ids),
    ...
    if len(all_non_meta_bbox_ids) > 1:
    ...
    new_annotations, max_id = create_annotations_from_bbox_ids(
        sorted_toplevel_node_ids, bbox_by_id, document_root_id, max_id
    )
    all_annotations += new_annotations
    for parent_id, children_ids in children_by_parent.items():
        ...
        new_annotations, max_id = create_annotations_from_bbox_ids(
            sorted_sibling_sequence, bbox_by_id, parent_id, max_id
        )
        all_annotations += new_annotations
