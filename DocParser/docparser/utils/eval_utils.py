...
def generate_obj_detection_results_based_on_directories(
    detFolder, gtFolder, detections_epoch_json_result_path, return_result_dict=False
):
    epoch_results = {"iou": dict()}
    logger.debug("gathering bounding boxes for : {}".format(detFolder))
    allBoundingBoxes, allClasses = getBoundingBoxes(gtFolder, BBFormat.XYX2Y2, "abs", isGT=True, header=True)
    ...
    ious = [0.5, 0.65, 0.8]
    for iou in ious:
        detections, matches_per_img = evaluator.GetPascalVOCMetrics(
            allBoundingBoxes, iou, method=MethodAveragePrecision.EveryPointInterpolation
        )
        epoch_results["iou"][str(iou)] = {
            "detections": detections,
            "matches_per_img": matches_per_img,
        }
...
def generate_relation_classification_results_based_on_directories(
    relation_files_dir,
    detections_epoch_json_result_path,
    relations_epoch_json_result_path,
    relations_gt_file,
    ...
    relations_results = get_matches_per_image(
        ious,
        detected_relations_per_image,
        epoch_detection_results,
...
def get_matches_per_image(
    ...
    for iou in ious:
        ...
        relations_gathered[iou] = {
            "detected_relations": set(detected_relations),
    ...
    for iou in ious:
        ...
        _, _, f1_followed_by = calculate_relation_scores(
            true_positives,
            false_positives,
            false_negatives,
            relation_type="COMES_BEFORE",
        )
        _, _, f1_parent_of = calculate_relation_scores(
            true_positives,
            false_positives,
            false_negatives,
        ...
        relations_results["iou"][iou] = {
            ...
            "recall": recall,
            "f1": f1,
            "f1_followed_by": f1_followed_by,
            "f1_parent_of": f1_parent_of,
...
def evaluate_relations_in_dict(relations_gathered):
    ...
    for (img_name, rel_obj_id, rel_subj_id, rel_class) in detected_relations:
        ...
        rel_obj_id_mapped = id_mappings_for_detected_objects[img_name].get(rel_obj_id, "NOMATCH_" + rel_obj_id)
        rel_subj_id_mapped = id_mappings_for_detected_objects[img_name].get(rel_subj_id, "NOMATCH_" + rel_subj_id)
...
def convert_bbox_list_to_save_format(all_bboxes_for_img):
    ...
    for b in all_bboxes_for_img:
        ...
        pred = {
            "pred_nr": b.getBboxID(),
            "class_name": b.getClassId(),
            "pred_score": b.getConfidence(),
...
def evaluate_icdar_xmls(detections_dir, gt_source_dir, eval_jar_path):
    ...
    try:
        ...
        f_score = 0
    results_dict = {
        "precisions": prec_per_doc,
        "recalls": rec_per_doc,
        "f1s": f1_per_doc,
        "avg_precision": prec_avg,
        "avg_recall": recall_avg,
        "f1": f_score,
        "f1_alternative": f1_avg,
    }
    logger.debug(
        "{}; {} total documents considered; avg precision: {}, avg recall: {}, avg F1: {}".format(
            detections_dir, len(prec_per_doc), prec_avg, recall_avg, f_score
        )
    )
    return results_dict
...
def keep_best_doc_if_multiple_gt_versions_exist(special_doc_id_mapping, prec_per_doc, rec_per_doc, f1_per_doc):
    for orig_doc_id, special_doc_ids in special_doc_id_mapping.items():
        ...
        logger.debug(
            "added best values prec/recall: {}/{} for key {}".format(
                prec_per_doc[orig_doc_id], rec_per_doc[orig_doc_id], orig_doc_id
            )
        )
        for special_doc_id in special_doc_ids:
