...
from docparser.objdetmetrics_lib.BoundingBoxes import BoundingBoxes, getBoundingBoxesForFile
from docparser.objdetmetrics_lib.utils import BBFormat

logging.config.fileConfig(os.path.join(os.path.dirname(__file__), "logging.conf"))
logger = logging.getLogger(__name__)

from docparser.utils.data_utils import DocsDataset, create_dir_if_not_exists
...
class StructureParser(object):
    ...
    def wrap_invalid_toplevel_bboxes(
        self,
        all_bboxes_for_img,
        ...
        if len(invalid_toplevel_bboxes) > 0:
            for invalid_toplevel_bbox in invalid_toplevel_bboxes:
                ...
                possible_parents = [
                    parent
                    for (parent, child) in HighlevelGrammar.get_allowed_hierarchies()
                    if child == invalid_category
                ]
                logger.debug(
                ...
                candidate_bboxes = [
                    x.getBboxID()
                    for x in all_bboxes_for_img
                    if x.getBboxID() in before_and_after_ids
                    and Evaluator.Evaluator.boxesIntersect(
    ...
    def create_icdar_structures_for_docs(
        self,
        detections_dir,
        output_xml_dir,
        ...
        process_detections_for_icdar_data(
            detections_dir,
            output_xml_dir,
            original_data_folder,
            crop_data_folder,
        ...
        for parent_id, child_id in same_category_nestings:
            if (
                len(
                    [
                        x
                        for x in children_by_parent[parent_id]
                        if id_to_class_mapping[x] == id_to_class_mapping[parent_id]
                    ]
                )
                == 1
            ):  # or  and id_to_class_mapping[child_id] not in HighlevelGrammar.allowed_to_be_nested():
                ...
                logger.debug(
                    "merging two nested bounding boxes ({}/{}) of same category: {}".format(
                        parent_id, child_id, id_to_class_mapping[parent_id]
                    )
                )
    ...
    def create_structure_for_doc(self, detection_result_file, table_mode=False, do_postprocessing=False):
        if table_mode:
            ...
            allBoundingBoxes, _ = getBoundingBoxesForFile(
            ...
            if do_postprocessing:
                ...
                while updated_bboxes is True:
                    ...
                    (
                        ...
                        meta_bboxes_for_img,
                        invalid_toplevel_bboxes,
                    ) = generate_relations_for_image(
                        all_bboxes_for_img, enforce_hierarchy=False
                    )  # also allow bad parent-child relations to be generated such that they can be potentially fixed
                    ...
                    (
                        updated_bboxes,
                        all_bboxes_for_img,
                    ) = self.align_parents_and_children(all_bboxes_for_img, is_parent_relations)
                    if updated_bboxes:
                        continue
                    ...
                    (
                        updated_bboxes,
                        all_bboxes_for_img,
                    ) = self.merged_direct_nesting_of_same_category(all_bboxes_for_img, is_parent_relations)
                    if updated_bboxes:
                        continue

                    updated_bboxes, all_bboxes_for_img = self.wrap_invalid_children(
                        all_bboxes_for_img, is_parent_relations, all_ids_for_img
                    )
                    if updated_bboxes:
                        continue
            ...
            (
                ...
                meta_bboxes_for_img,
                invalid_toplevel_bboxes,
            ) = generate_relations_for_image(all_bboxes_for_img)

            flat_annotations = create_flat_annotation_list(
                all_bboxes_for_img,
                meta_bboxes_for_img,
                is_parent_relations,
                sequence_relations,
            ...
            img_relations_dict = {
                "relations": is_parent_relations + sequence_relations,
                "flat_annotations": flat_annotations,
    ...
    def wrap_invalid_children(self, all_bboxes_for_img, is_parent_relations, all_ids_for_img):
        ...
        for parent_id, children_ids in children_by_parent.items():
            ...
            if rule == "SINGLE_OR_WITH" and cat1 in children_categories:
                if (
                    any(x != cat1 and x != cat2 for x in children_categories) or children_categories.count(cat1) > 1
                ):  # first condition: "are there any other children that are not of the allowed secondary category
    ...
    def create_gui_doc_entry(self, debug_gui_folder, img_relations_dict, img_path, out_tag="debugstruct-ws"):
        ...
        with open(dest_ann_path, "w") as out_file:
            json.dump(
                img_relations_dict["flat_annotations"],
                out_file,
                indent=4,
                sort_keys=True,
            )

        all_bb = BoundingBoxes()
        drawed_img = all_bb.drawAllBoundingBoxes(cv2.imread(dest_img_path), img_name, img_relations_dict["all_bboxes"])
        cv2.imwrite(dest_img_path, drawed_img)
...
class HighlevelGrammar(object):
    ...
    NONE = 3
    AT_LEAST_TWO = 4
    allowed_hierarchy_relations = {
        ("figure", "figure_caption"): ANY_NUMBER,
        ("figure", "figure_graphic"): AT_MOST_ONE,
        ("table", "tabular"): AT_LEAST_ONE,
        ("table", "table_caption"): ANY_NUMBER,
...
def assign_leftright_bboxes_to_y_ranges(left_bboxes, right_bboxes, sorted_leftright_y_ranges):
    ...
    for left_bbox in sorted_left_bboxes:
        ...
        if best_fit_range not in leftright_bboxes_by_range:
            leftright_bboxes_by_range[best_fit_range] = {
                "left": [],
                "right": [],
                "center": [],
            }
...
def sanity_check_grouped_bboxes(
    leftright_bboxes_by_y_range,
    center_bboxes_by_y_range,
    left_bboxes,
    right_bboxes,
    center_bboxes,
):
...
def group_by_column(unsorted_bboxes, width, height):
    ...
    leftright_bboxes_by_y_range = assign_leftright_bboxes_to_y_ranges(
        left_bboxes, right_bboxes, all_sorted_vertical_y_ranges
    )
    center_bboxes_by_y_range = assign_center_bboxes_to_y_ranges(center_bboxes, all_sorted_vertical_y_ranges)
    sanity_check_grouped_bboxes(
        leftright_bboxes_by_y_range,
        center_bboxes_by_y_range,
        left_bboxes,
        right_bboxes,
...
def determine_relation_matches(
    all_detected_relations,
    all_gt_relations,
    detection_to_gt_mapping,
    ...
    for relation_cand in all_relation_candidates:
        relation_with_gt_ids = (
            detection_to_gt_mapping[relation_cand[0]],
            detection_to_gt_mapping[relation_cand[1]],
            relation_cand[2],
        )
...
def unique_relation_in_loop_by_hierarchy(circular_rel_bboxes):
    ...
    if tuple(map(BoundingBox.getClassId, relation_option1)) in allowed_hierarchy_relations:
        definite_rel = (
            relation_option1[0].getBboxID(),
            relation_option1[1].getBboxID(),
        )
    if tuple(map(BoundingBox.getClassId, relation_option2)) in allowed_hierarchy_relations:
        if definite_rel is None:
            definite_rel = (
                relation_option2[0].getBboxID(),
                relation_option2[1].getBboxID(),
            )
        else:
        ...
        for circular_rel in circular_rels:
            ...
            if unique_relation_in_loop_by_hierarchy(circular_rel_bboxes) is not None:
                (
                    allowed_parent_id,
                    allowed_child_id,
                ) = unique_relation_in_loop_by_hierarchy(circular_rel_bboxes)
            ...
            # TODO: fallback: if two annotations are identical and circular, let annotation ID decide
                ...
                child_bboxes_by_parent = remove_relation_from_hierarchy_dict(child_bboxes_by_parent, bad_relation1)
                child_bboxes_by_parent = remove_relation_from_hierarchy_dict(child_bboxes_by_parent, bad_relation2)
...
def find_toplevel_bboxes(all_bboxes_for_img, cleaned_sibling_ids_by_parent):
    ...
    invalid_toplevel_bboxes = [
        bbox for bbox in all_toplevel_bboxes if bbox.getClassId() not in allowed_highlevel_classes
    ]
...
def ensure_unique_parents(child_bboxes_by_parent, id_to_bbox_mapping):
    ...
    if len(conflict_children) > 0:
        ...
        for conflict_child in conflict_children:
            ...
            maximum_overlap_indices = [
                i for i, x in enumerate(relative_overlaps) if x == max(relative_overlaps) and max(relative_overlaps) > 0
            ]
            ...
            if len(maximum_overlap_indices) == 1:
                ...
                logger.error(
                    "Could not find parent for overlaps: {}, confidences: {}, sizes: {}".format(
                        relative_overlaps, parent_confidences, parent_sizes
                    )
                )
                raise NotImplementedError
    ...
    for parent_id, child_ids in child_bboxes_by_parent.items():
        ...
        for child_id in child_ids:
            ...
            if class_relation in allowed_hierarchy_relations:
                ...
                logger.debug(
                    "Invalid child removed for parent {} ({}): {}".format(
                        parent_id, class_relation[0], class_relation[1]
                    )
                )
...
def get_cleaned_sibling_by_parent_bboxes(all_bboxes_for_img, enforce_hierarchy=False):
    ...
    cleaned_sibling_ids_by_parent_with_unique_parents = ensure_unique_parents(
        cleaned_sibling_ids_by_parent, id_to_bbox_mapping
    )
...
def get_gt_relations_and_filepath_mapping(eval_folder):
    ...
    img_filepath_by_filename = {
        os.path.basename(gt_img_info["path"]): gt_img_info["path"] for gt_img_info in gt_img_infos.values()
    }
...
def generate_relations_for_image(all_bboxes_for_img_including_meta, enforce_hierarchy=True):
    ...
    logger.debug(
        "excluded {} annotation bboxes of meta types".format(
            len(all_bboxes_for_img_including_meta) - len(all_bboxes_for_img)
        )
    )

    id_to_bbox_mapping = {bbox.getBboxID(): bbox for bbox in all_bboxes_for_img}
    cleaned_sibling_ids_by_parent = get_cleaned_sibling_by_parent_bboxes(
        all_bboxes_for_img, enforce_hierarchy=enforce_hierarchy
    )
    ...
    for parent_id, child_ids in cleaned_sibling_ids_by_parent.items():
        ...
        for child_id in child_ids:
            ...
            ids_relation = (
                parent_bbox.getBboxID(),
                child_bbox.getBboxID(),
                "IS_PARENT_OF",
            )
            is_parent_relations.append(ids_relation)

    valid_toplevel_bboxes, invalid_toplevel_bboxes = find_toplevel_bboxes(
        all_bboxes_for_img, cleaned_sibling_ids_by_parent
    )
    ...
    nested_sequence_relations = get_sibling_sequence_relations(
        cleaned_sibling_ids_by_parent, width, height, id_to_bbox_mapping
    )
    ...
    if len(all_bbox_ids) > 1:
        if not all_relation_ids == all_bbox_ids:
            logger.error(
                "Some bboxes have been left unannotated!: relation ids: {}, all bbox ids: {}".format(
                    all_relation_ids, all_bbox_ids
                )
            )
            raise AssertionError
