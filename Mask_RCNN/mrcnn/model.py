        ...
        try:
            ...
            try:
                ...
                if current_dataset.image_info[image_id]["source"] in no_augmentation_sources:
                    (image, image_meta, gt_class_ids, gt_boxes, gt_masks, _,) = load_image_gt(
                        current_dataset,
                        config,
                        image_id,
                        augment=augment,
                ...
                print(
                    "worker {} error reading image or annotations of {}. skipping..".format(
                        pid, current_dataset.image_info[image_id]["path"]
                    )
                )
                continue
            end = time.time()
            ...
            logging.exception("Error processing image {}".format(current_dataset.image_info[image_id]))
            error_count += 1
            if error_count > 5:
                raise
...
class MaskRCNN:
    ...
    def set_log_dir(self, model_path=None):
        ...
        self.log_dir = os.path.join(self.model_dir, "{}{:%Y%m%dT%H%M%S%f}".format(self.config.NAME.lower(), now))
    ...
    def train(
        ...
        train_generator = data_generator(
            ...
            extra_dataset=second_train_dataset,
        )
